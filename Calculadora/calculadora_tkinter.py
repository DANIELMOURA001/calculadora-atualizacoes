...
# Função para garantir que o arquivo versao.txt exista
def garantir_versao_txt():
    if not os.path.exists(VERSAO_LOCAL_TXT):
        with open(VERSAO_LOCAL_TXT, 'w') as f:
            f.write("0.0.0")  # Versão inicial padrão

# Verificar se o usuário deseja atualizar
garantir_versao_txt()  # Garantir que o arquivo de versão exista antes de verificar a atualização
